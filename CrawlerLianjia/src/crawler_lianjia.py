...
if __name__ == '__main__':
    ...
    #应对 反爬虫 机制
    # https://blog.csdn.net/qq_32740675/article/details/79720367
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36',
    }

    res = requests.get('http://bj.lianjia.com/ershoufang',headers=headers)

    res = res.text.encode(res.encoding).decode('utf-8')
    soup = BeautifulSoup(res, 'html.parser')
    print soup.prettify()
    # 通过 游览器 F12 找到 网页中元素 对应的代码段（可以将它复制到一个 html的文档中方便查看 ），并在代码段中找到关键词
